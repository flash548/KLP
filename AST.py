...
class IfNode(AST):
    ...
    def __init__(self, cond, if_body, else_if_conds, else_if_bodies, else_body):
        self._cond = cond
    ...
    def emit(self, vm):
        ...
        for i in self._if_body: i.emit(vm)
        ...
        if len(self._else_if_conds) > 0:
            for else_if_cond in self._else_if_conds:
                ...
                vm.pgm_stack[else_if_anchor] = Instruction("BZ", [ else_if_clause_anchor ])
                
        # do the else clause now
        for else_clause in self._else_body:
            else_clause.emit(vm)
            
        # go back and populate our JMPs since we know where the IF clause ends now...
...
class AssignNode(AST):
    ...
    def emit(self, vm):
        vm.append_instruction(Instruction("ASSIGN", [ self._name, self._context ]))
